    ...
    <div style={{ display:"flex", minHeight:"100vh", background:T.bg, color:T.text, fontFamily:"'DM Sans','Apple SD Gothic Neo',sans-serif" }}>
      ...
      <aside style={{ width:220, background:T.surface, borderRight:`1px solid ${T.border}`, display:"flex", flexDirection:"column", padding:"24px 0", flexShrink:0 }}>
        ...
        <nav style={{ padding:"14px 10px", flex:1, display:"flex", flexDirection:"column", gap:2 }}>
          {[{id:"dashboard",icon:"⊞",label:"대시보드"},{id:"new_project",icon:"+",label:"새 프로젝트"},{id:"ossp",icon:"◈",label:"OSSP 라이브러리"}].map(item=>(
      ...
      <main style={{ flex:1, overflow:"auto", animation:"fadeIn .3s ease" }}>{pages[page]||pages.dashboard}</main>
    </div>
  );
}
  ...
  return (
    <div style={{ padding:"36px 40px", maxWidth:1100 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:28 }}>
        {stats.map(s=><Card key={s.label} style={{ padding:"18px 22px" }}><div style={{ fontSize:11, color:T.muted, marginBottom:6 }}>{s.label}</div><div style={{ fontSize:30, fontWeight:700, color:s.color, lineHeight:1 }}>{s.value}</div></Card>)}
      </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
              {projects.map(p=>(
                <div key={p.id} onClick={()=>{ setCurrentProject(p); nav("project_detail"); }}
                  ...
                  <div><div style={{ fontWeight:600, marginBottom:2 }}>{p.name}</div><div style={{ fontSize:11, color:T.muted }}>{p.client} · {p.ossp?.label} · {p.createdAt}</div></div>
  ...
  return (
    <div style={{ padding:"32px 36px", maxWidth:960 }}>
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:28 }}>
        ...
        <div><h1 style={{ fontSize:20, fontWeight:700 }}>새 프로젝트 생성</h1><p style={{ fontSize:12, color:T.muted }}>STEP {step+1} / {steps.length} — {steps[step]}</p></div>
      </div>
      <div style={{ display:"flex", gap:0, marginBottom:32, position:"relative" }}>
        ...
        {steps.map((s,i)=>(
          <div key={s} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:5, position:"relative", zIndex:2 }}>
            <div style={{ width:26, height:26, borderRadius:"50%", background:i<step?T.accent:i===step?T.accent:T.surface, border:`2px solid ${i<=step?T.accent:T.border}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:700, color:i<=step?"#fff":T.muted, transition:"all .3s" }}>{i<step?"✓":i+1}</div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", marginTop:18 }}>
        ...
        {step<steps.length-1 ? <Btn disabled={!canNext[step]} onClick={()=>setStep(s=>s+1)}>다음 →</Btn> : <Btn onClick={onFinish}>✓ 프로젝트 생성 완료</Btn>}
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:18 }}>
        <div><h2 style={{ fontSize:17, fontWeight:600, marginBottom:4 }}>PDP 자동 생성</h2><p style={{ fontSize:12, color:T.muted }}>AI가 프로젝트 개발 계획서를 자동으로 작성합니다.</p></div>
      ...
      {pdpData && (
        <div style={{ display:"flex", flexDirection:"column", gap:12, animation:"fadeIn .4s" }}>
          ...
          <Card style={{ padding:16, background:T.bg }}><div style={{ fontSize:11, color:T.muted, marginBottom:5, fontWeight:600 }}>목적 / 범위</div><div style={{ fontSize:12, color:T.text, lineHeight:1.7 }}>{pdpData.overview?.purpose}</div></Card>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            <Card style={{ padding:16, background:T.bg }}>
              ...
              {pdpData.schedule?.phases?.map((ph,i)=><div key={i} style={{ fontSize:11, color:T.text, padding:"4px 0", borderBottom:`1px solid ${T.border}` }}><span style={{ color:T.accent, marginRight:6 }}>■</span>{ph.phase} <span style={{ color:T.muted }}>({ph.start?.slice(5)}~{ph.end?.slice(5)})</span></div>)}
            </Card>
            <Card style={{ padding:16, background:T.bg }}>
              <div style={{ fontSize:11, color:T.muted, marginBottom:8, fontWeight:600 }}>위험 요소</div>
              {pdpData.risks?.map((r,i)=><div key={i} style={{ fontSize:11, padding:"4px 0", borderBottom:`1px solid ${T.border}`, display:"flex", justifyContent:"space-between" }}><span style={{ color:T.text }}>{r.description?.substring(0,28)}…</span><Badge color={r.level==="상"?T.red:r.level==="중"?T.amber:T.green}>{r.level}</Badge></div>)}
            </Card>
          </div>
        </div>
      )}
...
function StepWBS({ wbsData, generating, genError, onGenerate }) {
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:18 }}>
        <div><h2 style={{ fontSize:17, fontWeight:600, marginBottom:4 }}>WBS 자동 생성</h2><p style={{ fontSize:12, color:T.muted }}>AI가 작업 분류 구조를 자동으로 생성합니다.</p></div>
            ...
            {wbsData.tasks?.map(t=>(
              <Card key={t.id} style={{ padding:14, background:T.bg }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:3 }}>
                  {t.subtasks?.map(s=><div key={s.id} style={{ display:"flex", alignItems:"center", gap:8, padding:"4px 9px", background:T.surface, borderRadius:7, fontSize:11 }}><span style={{ color:T.muted, fontFamily:"monospace", fontSize:10 }}>{s.wbsCode}</span><span style={{ flex:1 }}>{s.task}</span><span style={{ color:T.muted }}>{s.duration}</span><Badge color={s.status==="완료"?T.green:s.status==="진행"?T.amber:T.muted}>{s.status}</Badge></div>)}
                </div>
              </Card>
            ))}
          </div>
...
function StepDeliverables({ deliverablesData, generating, genError, onGenerate }) {
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:18 }}>
        <div><h2 style={{ fontSize:17, fontWeight:600, marginBottom:4 }}>착수/계획 산출물 자동생성</h2><p style={{ fontSize:12, color:T.muted }}>AI가 SI 프로젝트 착수에 필요한 초기 문서 패키지를 자동 구성합니다.</p></div>
      ...
      {deliverablesData && (
        <div style={{ animation:"fadeIn .4s" }}>
          ...
          <div style={{ display:"flex", gap:9, marginBottom:12 }}>
            {[{label:"전체 산출물",value:deliverablesData.summary?.totalDocs,color:T.accent},{label:"필수 문서",value:deliverablesData.summary?.mandatoryCount,color:T.red},{label:"예상 소요일",value:deliverablesData.summary?.estimatedDays+"일",color:T.amber}].map(s=>(
      ...
      {tab==="overview" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
          {[["기간",`${project.startDate} ~ ${project.endDate}`],["유형",project.type],["OSSP",project.ossp?.label],["문서화 수준",project.tailoring?.doc_level],["리뷰 주기",project.tailoring?.review_cycle],["테스트 수준",project.tailoring?.test_level]].map(([k,v])=>(
            <Card key={k} style={{ padding:"14px 18px" }}><div style={{ fontSize:10, color:T.muted, marginBottom:5, textTransform:"uppercase", letterSpacing:0.5 }}>{k}</div><div style={{ fontSize:14, fontWeight:600 }}>{v}</div></Card>
          ))}
        </div>
      )}
      {tab==="pdp" && project.pdp && (
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          <Card style={{ padding:18 }}><div style={{ fontSize:11, color:T.muted, marginBottom:7, fontWeight:600 }}>목적</div><div style={{ fontSize:13, lineHeight:1.7 }}>{project.pdp.overview?.purpose}</div></Card>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            <Card style={{ padding:18 }}><div style={{ fontSize:11, color:T.muted, marginBottom:8, fontWeight:600 }}>일정 계획</div>{project.pdp.schedule?.phases?.map((ph,i)=><div key={i} style={{ fontSize:11, padding:"5px 0", borderBottom:`1px solid ${T.border}` }}><span style={{ color:T.accent, fontWeight:600 }}>{ph.phase}</span><div style={{ color:T.muted }}>{ph.start} ~ {ph.end}</div></div>)}</Card>
            <Card style={{ padding:18 }}><div style={{ fontSize:11, color:T.muted, marginBottom:8, fontWeight:600 }}>위험 관리</div>{project.pdp.risks?.map((r,i)=><div key={i} style={{ fontSize:11, padding:"5px 0", borderBottom:`1px solid ${T.border}` }}><div style={{ display:"flex", justifyContent:"space-between" }}><span>{r.description?.substring(0,24)}…</span><Badge color={r.level==="상"?T.red:r.level==="중"?T.amber:T.green}>{r.level}</Badge></div></div>)}</Card>
          </div>
        </div>
      )}
      {tab==="wbs" && project.wbs && (
          ...
          {project.wbs.tasks?.map(t=>(
            <Card key={t.id} style={{ padding:16 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:5 }}>
                {t.subtasks?.map(s=><div key={s.id} style={{ display:"flex", alignItems:"center", gap:7, padding:"6px 10px", background:T.bg, borderRadius:7, fontSize:11 }}><span style={{ color:T.muted, fontFamily:"monospace", fontSize:9 }}>{s.wbsCode}</span><span style={{ flex:1 }}>{s.task}</span><Badge color={s.status==="완료"?T.green:s.status==="진행"?T.amber:T.muted}>{s.status}</Badge></div>)}
              </div>
            </Card>
          ))}
        </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div style={{ display:"flex", gap:11 }}>
            {[{label:"전체 산출물",value:project.deliverables.summary?.totalDocs,color:T.accent},{label:"필수 문서",value:project.deliverables.summary?.mandatoryCount,color:T.red},{label:"예상 소요일",value:project.deliverables.summary?.estimatedDays+"일",color:T.amber}].map(s=>(
              <Card key={s.label} style={{ flex:1, padding:"13px 16px", textAlign:"center" }}><div style={{ fontSize:10, color:T.muted, marginBottom:3 }}>{s.label}</div><div style={{ fontSize:22, fontWeight:700, color:s.color }}>{s.value}</div></Card>
            ))}
          </div>
          {project.deliverables.categories?.map(cat=>(
            <Card key={cat.id} style={{ padding:18 }}>
              <div style={{ display:"flex", alignItems:"center", gap:9, marginBottom:12 }}><span style={{ fontSize:18 }}>{cat.icon}</span><span style={{ fontWeight:700, fontSize:14 }}>{cat.name}</span><span style={{ fontSize:11, color:T.muted, marginLeft:"auto" }}>{cat.documents?.length}건</span></div>
              <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
                {cat.documents?.map(doc=>(
                  <div key={doc.id} style={{ display:"flex", alignItems:"flex-start", gap:9, padding:"9px 11px", background:T.bg, borderRadius:9 }}>
                    ...
                    <div style={{ flex:1 }}><div style={{ fontSize:12, fontWeight:600, marginBottom:2 }}>{doc.name}</div><div style={{ fontSize:10, color:T.muted, marginBottom:2 }}>{doc.purpose}</div><div style={{ fontSize:10, color:T.muted }}>📄 {doc.template}</div></div>
...
function OSSPPage({ nav }) {
  return (
    <div style={{ padding:"32px 36px", maxWidth:900 }}>
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:24 }}>
        <button onClick={()=>nav("dashboard")} style={{ background:"none", border:"none", color:T.muted, cursor:"pointer", fontSize:20 }}>←</button>
        <div><h1 style={{ fontSize:20, fontWeight:700 }}>OSSP 라이브러리</h1><p style={{ fontSize:12, color:T.muted }}>지원되는 개발 방법론 및 단계 구성</p></div>
      </div>
      <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
        {OSSP_OPTIONS.map(o=>(
          <Card key={o.id} style={{ padding:22 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14 }}>
              <div><div style={{ fontSize:17, fontWeight:700, marginBottom:3 }}>{o.label}</div><div style={{ fontSize:12, color:T.muted }}>{o.desc}</div></div>
            ...
            <div style={{ display:"flex", gap:7, flexWrap:"wrap" }}>
              {o.phases.map((ph,i)=>(
                <div key={ph} style={{ display:"flex", alignItems:"center", gap:5 }}>
                  <div style={{ padding:"5px 12px", background:T.bg, border:`1px solid ${T.border}`, borderRadius:7, fontSize:12 }}><span style={{ color:T.accent, marginRight:5 }}>{i+1}.</span>{ph}</div>
